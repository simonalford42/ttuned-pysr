"""
Training script for autoregressive symbolic regression prediction.
Trains a transformer to predict the entire next generation autoregressively given current population.
"""
import argparse
import json
import os
...
def main(args):
    ...
    def tokenize(element):
        """
        Tokenize context/population/target triplets for autoregressive learning.
        For autoregressive mode: target is the entire next population to be generated token-by-token.
        ...
        for i in range(len(element["context"])):
            ...
            # Create input part (context + population + <TARGET> prompt)
            input_part = format_input_part(tokenizer.bos_token, context, population)

            # Create target part (entire next population with fitnesses)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, help="Path to config file", default='training/configs/autoreg-s.json')
